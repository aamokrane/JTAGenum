...
void change_number_of_pins()
{
  //Change the number of pins that the we use
  // READ The typed number of pins
  Serial.print("Type in the number of pins : ");
  //delay(3000); // hoping read buffer is idle after 5 ms
  ...
  while (Serial.available() && i < 2)
  {
    ...
    if(c > '9' || c < '0')
    {
      Serial.println("The number of pins must contain only digits");
      return;
    }
    nb = (nb*10) + (c - '0');
    i++;
  ...
  Serial.flush();
  pinslen = nb;
  Serial.print("The number of pins ghave been changed to ");
  Serial.println(pinslen);
}
...
void help()
{
	...
	printProgStr(PSTR("]\r\n"
			...
			"	 look at the main()/loop() code to specify pins.\r\n"
      "n > number pins\r\n" 
      "  change the numner of pins to test (4-10)"
			));
}
/*
 * main()
...
void loop() 
{
    ...
    //Serial.print("Just received this command : ");
    //Serial.println(command); // echo the command
		...
		else if(strcmp(command, "pullups") == 0                          || strcmp(command, "r") == 0)
		{
			...
			help();
    else if(strcmp(command, "number pins") == 0                             || strcmp(command, "n") == 0)
      {
        change_number_of_pins();
      }
		else 
		{
